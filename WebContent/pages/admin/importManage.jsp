...
<script type="text/javascript">
	...
	function openImportAddDialog(){
		$("#dlgadd").dialog("open").dialog("setTitle","添加入库信息");
	...
	function openImportModifyDialog(){
		...
		$("#dlgadd").dialog("open").dialog("setTitle","编辑入库信息");
		$("#goodsId").combobox("setValue",row.goodsId);
		$("#impoWarehouseId").val(row.impoWarehouseId);
		$("#impoDate").datebox("setValue",row.impoDate);
		$("#impoNum").val(row.impoNum);
		$("#impoDes").val(row.impoDes);
		$("#providerId").combobox("setValue",row.providerId);
...
<body style="margin: 5px;">
	 ...
	 pagination="true" rownumbers="true" url="${pageContext.request.contextPath}/stockManageSystem/import" toolbar="#tb">
		<thead>
			<tr>
				...
				<th field="typeName" width="30">货物名称</th>
				<th field="impoWarehouseId" width="30">入库仓库</th>
				<th field="impoDate" width="60">入库时间</th>
				<th field="impoNum" width="60">入库数量</th>
				<th field="proName" width="100">归属客户</th>
				<th field="impoDes" width="100">入库备注</th>
	...
	<div id="dlgadd" class="easyui-dialog" style="width:580px;height:350px;padding: 10px 20px"
	closed="true" buttons="#dlg-buttons">
		<form id="fm" method="post">
			<table cellspacing="5px;">
				<tr>
					<td>货物名称：</td>
					<td><input class="easyui-combobox" name="importGoods.goodsId" id="goodsId" size="12" data-options="panelHeight:'200px',editable:false,valueField:'id',textField:'typeName',url:'${pageContext.request.contextPath}/stockManageSystem/goodsType!goodsTypeComboList'"/></td>
					
					<td>入库仓库：</td>
					<td><input type="text" name="importGoods.impoWarehouseId" id="impoWarehouseId" class="easyui-validatebox" /></td>
				</tr>
				<tr>
					<td>入库日期：</td>
				...
				<tr>
					<td>所属客户：</td>
					<td><input class="easyui-combobox" name="importGoods.providerId" id="providerId" data-options="panelHeight:'200px',editable:false,valueField:'id',textField:'proName',url:'${pageContext.request.contextPath}/stockManageSystem/provider!providerComboList'"/></td>
					<td> </td>
					<td></td>
				</tr>
				<tr>
					<td valign="top">入库备注：</td>
					<td colspan="3"><textarea rows="5" cols="52" name="importGoods.impoDes"  id="impoDes"></textarea></td>
				</tr>
			</table>
		</form>
	</div>
						...
						<table style="height:250px; width:540px" id="dg2" title="商品选择" class="easyui-datagrid" fitColumns="true"
	 							...
	 							<thead>
									<tr>
										...
										<th field="goodsId" width="10" hidden="true">货物编号</th>
										<th field="goodsName" width="70">货物名称</th>
										<th field="impoWarehouse" width="70">入库仓库</th>
