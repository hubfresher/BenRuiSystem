...
//			s_typeId:$('#s_typeId').combobox("getValue")
	...
	function deleteDriver(){
		...
		$.messager.confirm("系统提示","您确认要删掉这<font color=red>"+selectedRows.length+"</font>条数据吗？",function(r){
			if(r){
				$.post("${pageContext.request.contextPath}/stockManageSystem/drivers!delete",{delId:ids},function(result){
					if(result.success){
	...
	function openDriversAddDialog(){
		$("#dlg").dialog("open").dialog("setTitle","添加司机信息");
		url="${pageContext.request.contextPath}/stockManageSystem/drivers!save";
	}
	...
	function openDriversModifyDialog(){
		var selectedRows=$("#dg").datagrid('getSelections');
		if(selectedRows.length!=1){
		...
		url="${pageContext.request.contextPath}/stockManageSystem/drivers!save?id="+row.id;
	}
	...
	function exportData(){
		window.open('${pageContext.request.contextPath}/stockManageSystem/drivers!export')
	}
	...
	function downloadTemplate(){
		window.open('${pageContext.request.contextPath}/template/driversTemp.xls');
	}
...
<body style="margin: 5px;">
	<table style="height:423px; width:1160px" id="dg" title="司机管理" class="easyui-datagrid" fitColumns="true"
	 pagination="true" rownumbers="true" url="${pageContext.request.contextPath}/stockManageSystem/drivers" toolbar="#tb">
		<thead>
			<tr>
				<th field="cb" checkbox="true"></th>
				<th field="id" width="15">编号</th>
				<th field="name" width="15" >司机姓名</th>
				<th field="phone" width="25">手机号</th>
				<!--  
				...
				-->
				<th field="born" width="25">籍贯</th>
				<th field="car" width="100">车号</th>
			</tr>
		</thead>
	</table>
	...
	<div id="tb">
		<div>
			<a href="javascript:openDriversAddDialog()" class="easyui-linkbutton" iconCls="icon-add" plain="true">添加</a>
			<a href="javascript:openDriversModifyDialog()" class="easyui-linkbutton" iconCls="icon-edit" plain="true">修改</a>
			<a href="javascript:deleteDriver()" class="easyui-linkbutton" iconCls="icon-remove" plain="true">删除</a>
		...
		<div>&nbsp;司机姓名：&nbsp;<input type="text" name="s_drivername" id="s_drivername" size="10" />
		...
		<!--  &nbsp;供应商：&nbsp;<input class="easyui-combobox" id="s_proId" name="s_proId" size="10" data-options="panelHeight:'auto',editable:false,valueField:'id',textField:'proName',url:'${pageContext.request.contextPath}/stockManageSystem/provider!providerComboList'"/>
			...
			<a href="javascript:searchDrivers()" class="easyui-linkbutton" iconCls="icon-search" plain="true">搜索</a>
	...
	closed="true" buttons="#dlg-buttons">
		<form id="fm" method="post">
			<table cellspacing="5px;">
				<tr>
					<td>司机姓名：</td>
					<td><input type="text" name="dirver.name" id="name" class="easyui-validatebox" required="true"/></td>
					
					<td>手机号：</td>
					<td><input type="text" name="dirver.phone" id="phone" /></td>
				</tr>
				<!--  
				<tr>
					<td>供应商：</td>
				...
				<tr>
					<td valign="top">籍贯：</td>
					<td><input type="text" name="dirver.born" id="born" /></td>
					
					<td valign="top">车号：</td>
					<td><input type="text" name="dirver.car" id="car"/></td>
				</tr>
	...
	<div id="dlg-buttons">
		<a href="javascript:saveDriver()" class="easyui-linkbutton" iconCls="icon-ok">保存</a>
		<a href="javascript:closeDriversDialog()" class="easyui-linkbutton" iconCls="icon-cancel">关闭</a>
	</div>
